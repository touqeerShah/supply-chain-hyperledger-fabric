...
import { RawMaterialEntity, RawMaterialEntityCount } from "../../rawMaterial/RawMaterial.entity";
 ...
 */
export class BaseRepository<T extends RawMaterialEntity> {
    ...
    /**
      ...
      * transction before add into blockchain
      */
    public async getRawMaterialEntityCount(privateCollection: string): Promise<RawMaterialEntityCount> {
   ...
   */
    ...
    public async update(updatedData: T, privateCollection: string): Promise<T | string> {
        ...
        const { rawMaterialId: modelId } = updatedData;
        if (modelId.length === 0) {
