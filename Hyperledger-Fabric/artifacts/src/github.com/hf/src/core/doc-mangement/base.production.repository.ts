...
import { ProductionEntity, ProductionEntityCount } from "../../production/production.entity";
 ...
 */
export class BaseRepository<T extends ProductionEntity> {
 ...
 */
    ...
    public async exists(query: T): Promise<boolean> {
        ...
        const bufferData = await this.contextProvider.get(key, "Production" + mspID);
    ...
    /**
      ...
      * transction before add into blockchain
      */
    public async getProductionEntityCount(privateCollection: string): Promise<ProductionEntityCount> {
        const bufferData = await this.contextProvider.get("productionsCount", privateCollection);
        let documentCount: ProductionEntityCount;
        if (bufferData.length === 0) {
   ...
   * transction before add into blockchain
   */
    public async updateProductionEntityCount(privateCollection: string) {

        let documentCount: ProductionEntityCount = await this.getProductionEntityCount(privateCollection);
        documentCount.count++
        ...
        // here we call function to put data into blockchain
        await this.contextProvider.put("productionsCount", buffer, privateCollection);
    }
    ...
    public async update(updatedData: T, privateCollection: string): Promise<T | string> {
        ...
        const { batchesNumber: modelId } = updatedData;
        if (modelId.length === 0) {
